...
<form method="post" action="" class="form layui-form">
  <table>
    ...
    <tr class="pro1 product_ids1"><td>选择产品：</td><td><input type="hidden" name="product_ids1" value="{$One['product_ids']}"><input type="hidden" name="product_selected" value="{$One['product_selected']}"><div class="product_select1 xm-select"></div></td><td>生成的下单页只显示勾选且在<a href="javascript:" onclick="window.parent.addTab('{:url('/product/index')}','产品管理')">产品管理</a>模块中设置了前台显示的产品</td></tr>
    <tr class="pro1"><td>默认产品：</td><td><select name="selected1" lay-search></select></td></tr>
    <tr class="pro2 product_ids2"><td>选择产品：</td><td><input type="hidden" name="sort2"><input type="hidden" name="product_ids2" value="{$One['product_ids']}"><div class="product_select2 xm-select"></div></td><td>生成的下单页只显示勾选且在<a href="javascript:" onclick="window.parent.addTab('{:url('/product/index')}','产品管理')">产品管理</a>模块中设置了前台显示的产品</td></tr>
    <tr class="pro2"><td>默认产品：</td><td><select name="selected2" lay-search></select></td></tr>
    <tr><td>产品显示：</td><td><div class="radio-box"><label><input type="radio" name="view_type" value="1" {if condition="$One['view_type']==1"}checked{/if}>单选按钮</label></div><div class="radio-box"><label><input type="radio" name="view_type" value="0" {if condition="$One['view_type']==0"}checked{/if}>下拉框</label></div></td></tr>
    ...
    <tr><td>订单查询：</td><td><div class="radio-box"><label><input type="radio" name="is_show_search" value="0" {if condition="$One['is_show_search']==0"}checked{/if}>关闭</label></div><div class="radio-box"><label><input type="radio" name="is_show_search" value="1" {if condition="$One['is_show_search']==1"}checked{/if}>开启</label></div></td><td>在<a href="javascript:" onclick="window.parent.addTab('{:url('/system/index')}','系统设置')">系统设置</a>模块中开启“订单查询”后，此设置才可生效</td></tr>
