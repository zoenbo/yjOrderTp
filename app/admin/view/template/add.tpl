...
<form method="post" action="" class="form">
  <table>
    ...
    <tr><td>指定代理：</td><td><select name="uid" class="select"><option value="0">不指定</option>{$Manager}</select></td></tr>
    <tr><td>使用模板：</td><td><select name="template" class="select">{$Template}</select></td><td><span class="view"></span>　　经典版不支持皮肤样式的定义</td></tr>
    <tr class="style"><td>皮肤样式：</td><td><select name="sid" class="select">{$Style}</select></td><td>手机版2和手机版3推荐使用11号皮肤样式，手机版4推荐使用12号皮肤样式</td></tr>
    ...
    <tr class="pro1"><td>选择分类：</td><td><select name="sort1" class="select">{$Sort}</select></td></tr>
