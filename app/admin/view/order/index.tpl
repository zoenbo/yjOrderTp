...
{block name="basecss"}<link rel="stylesheet" type="text/css" href="public/admin/styles/Order.css">{/block}
...
<div class="tools">
  ...
  <form method="post" action="" class="multi_form">
    ...
    <div class="radio-box"><label><input type="radio" name="type" value="5">修改订单状态：</label></div><select name="ostate" class="select">{$Ostate}</select>
    {/if}
...
<div class="list">
  <table>
    <tr><th class="all"><div class="radio-box"><label><input type="checkbox" class="all">全选</label></div></th><th class="oid">订单号</th><th class="admin">下单管理员</th><th class="template">下单模板</th><th class="name">姓名</th><th class="pro">订购产品</th><th class="price">成交单价</th><th class="count">订购数量</th><th class="total">成交总价</th><th class="tel">联系电话</th><th class="address">详细地址</th><th class="post">邮政编码</th><th class="email">电子邮箱</th><th class="ip">下单IP</th><th class="referrer">下单来路</th><th class="date">下单时间</th><th class="pay">支付状态</th><th class="pay_oid">支付订单号</th><th class="pay_scene">支付场景</th><th class="pay_date">支付时间</th><th class="state">订单状态</th><th class="logistics">物流信息</th><th class="control">操作</th></tr>
    {foreach name="All" key="key" item="value"}
    <tr><td><div class="radio-box"><label><input type="checkbox" name="id" value="{$value['id']}"><span>.</span></label></div></td><td>{$value['oid']|keyword}</td><td>{$value['admin']}</td><td>{$value['template']}</td><td>{$value['name']|keyword}</td><td>{$value['pro']}</td><td>{$value['price']}元</td><td>{$value['count']}</td><td>{$value['total']}元</td><td>{$value['tel']|keyword}</td><td title="{$value['address']}">{$value['address']|truncate=0,25|keyword}</td><td>{$value['post']|keyword}</td><td>{$value['email']|keyword}</td><td title="{$value['ipaddr']}">{$value['ip']|keyword}</td><td>{if condition="$value['referrer']"}<a href="{$value['referrer']}" target="_blank" title="{$value['referrer']}">来路</a>{else/}直接进入{/if}</td><td>{$value['date']|dateFormat}</td><td>{$value['pay']}</td><td>{$value['pay_oid']|keyword}</td><td>{$value['pay_scene']}</td><td>{if condition="$value['pay_date']"}{$value['pay_date']|dateFormat}{/if}</td><td>{$value['state']}</td><td>{$value['loginame']}<br><a href="http://www.kuaidi100.com/chaxun?com={$value['logicode']}&nu={$value['logistics_id']}" target="_blank">{$value['logistics_id']|keyword}</a></td><td><a href="{:url('/'.parse_name(request()->controller()).'/detail',['id'=>$value['id']])}">详情</a>/<a href="{:url('/'.parse_name(request()->controller()).'/update',['id'=>$value['id'],'from'=>0])}">修改</a>/{if condition="request()->controller()=='Order'"}<a href="{:url('/'.parse_name(request()->controller()).'/recycle',['id'=>$value['id']])}">删除</a>{else/}<a href="{:url('/'.parse_name(request()->controller()).'/recover',['id'=>$value['id']])}">还原</a>/<a href="{:url('/'.parse_name(request()->controller()).'/delete',['id'=>$value['id']])}">删除</a>{/if}</td></tr>
     {/foreach}
  </table>
</div>
{$Page}
...
<script type="text/javascript" src="public/admin/js/Order.js"></script>
{/block}