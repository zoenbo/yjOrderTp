...
<form method="post" action="" class="form layui-form">
  <table>
    ...
    <tr><td>订购产品：</td><td><select name="product_id" lay-filter="product_id" lay-search>{$Product}</select></td></tr>
    ...
    <tr class="aa"><td></td><td><input type="hidden" name="province"><select class="province" lay-filter="province" lay-search><option value="0">省份</option></select></td></tr>
    <tr class="aa"><td></td><td><input type="hidden" name="city"><select class="city" lay-filter="city" lay-search><option value="0">城市</option></select></td></tr>
    <tr class="aa"><td></td><td><input type="hidden" name="county"><select class="county" lay-filter="county" lay-search><option value="0">区/县</option></select></td></tr>
    <tr class="aa"><td></td><td><input type="hidden" name="town"><select class="town" lay-filter="town" lay-search><option value="0">乡镇/街道（若不清楚，可不选）</option></select></td></tr>
    ...
    <tr><td>支付状态：</td><td><select name="pay" lay-search>{foreach name="Pay" key="key" item="value"}<option value="{$key}">{$value}</option>{/foreach}</select></td></tr>
