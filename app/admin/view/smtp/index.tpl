...
{if condition="$Admin['level']==1||in_array($Permission[request()->controller()]['state'],$AdminPermit)"}<li><a href="{:url('/'.parse_name(request()->controller()).'/state')}">运行状态</a></li>{/if}
{/block}
...
<table class="tip">
  ...
  <tr><td class="num">2、</td><td>建议使用QQ邮箱，但注意：①要开通SMTP服务，开通方法请<a href="http://service.mail.qq.com/cgi-bin/help?subtype=1&no=166&id=28" target="_blank">点击此处</a>查看 ②“发信人邮件地址”和“身份验证用户名”请不要和<a href="javascript:;" onclick="addTab('{:url('/system/index')}','系统设置')">系统设置</a>模块中的管理员邮箱重复；</td></tr>
