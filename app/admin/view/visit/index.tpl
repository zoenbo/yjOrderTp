...
<table class="tip">
  ...
  <tr><td class="num">2、</td><td>为了节省数据库空间，系统会每天自动将统计的数据导出到文件，请到<a href="javascript:" onclick="window.parent.addTab('{:url('/output/index')}','导出的数据')">导出的数据</a>模块中进行下载。您也可以点击上方的“导出并清空”自行导出数据；</td></tr>
  <tr><td class="num">3、</td><td>如果发生统计不到的情况，请尝试<a href="{:url('/'.parse_name(request()->controller()).'/js')}">点击此处</a>配置Visit.js文件。</td></tr>
</table>
{/block}
