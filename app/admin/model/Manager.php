...
class Manager extends Model{
	...
	public function modify(){
		...
		if ($validate->only($scene)->check($data)){
			if (Request::post('pass')) $data['pass'] = sha1(Request::post('pass'));
			if ($this->repeat(true)) return '此帐号已存在！';
			unset($data['repass']);
	...
	public function modify2(){
		...
		$scene = [];
		if (Request::post('pass')){
			if (passEncode(Request::post('oldpass')) != $object['pass']) return '请输入正确的旧密码！';
			$data['pass'] = Request::post('pass');
			$data['repass'] = Request::post('repass');
			$scene[] = 'pass';
			$scene[] = 'repass';
		}
		if (Request::post('qqau')) $data['qqau'] = '';
		if (empty($data)) return '您没有做任何修改！';
		$validate = new valid();
		if ($validate->only($scene)->check($data)){
			$data['pass'] = passEncode(Request::post('pass'));
			unset($data['repass']);
	...
	public function modify3(){
		if (!Request::post('reset_pass_key')) return '密码重置密钥不得为空！';
		if (Request::post('reset_pass_key') != Config::get('system.reset_pass_key')) return '密码重置密钥不正确！';
		$data = [
			'pass'=>Request::post('pass'),
			'repass'=>Request::post('repass')
		];
