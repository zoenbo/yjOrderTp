...
class Manager extends Base
{
    ...
    public function index()
    {
        ...
        if ($object) {
            ...
            foreach ($object as $key => $value) {
                $object[$key]['level_name'] = '<span class="' . $this->level[$value['level']][0] . '">' .
                    $this->level[$value['level']][1] . '</span>';
                $object[$key]['order_permit'] = $value['level'] == 1 ? '-' : '<span class="' .
                    $this->orderPermit[$value['order_permit']][0] . '">' .
                    $this->orderPermit[$value['order_permit']][1] . '</span>';
    ...
    public function add()
    {
        if (Request::isPost()) {
            ...
            if (is_numeric($object)) {
                return $object > 0 ?
                    $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '管理员添加成功！') :
                    $this->failed('管理员添加失败！');
            } else {
    ...
    public function update()
    {
        if (Request::get('id')) {
            ...
            $object = $Manager->one();
            if (!$object) {
                return $this->failed('不存在此管理员！');
            }
            if (Request::isPost()) {
                if (Config::get('app.demo') && Request::get('id') == 1) {
                    return $this->failed('演示站，id为1的管理员无法修改！');
                }
                $object = $Manager->modify();
                return is_numeric($object) ?
                    $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '管理员修改成功！') :
                    $this->failed($object);
            }
    ...
    public function isActivation()
    {
        if (Request::get('id')) {
            ...
            if ($object['is_activation'] == 0) {
                if (!$Manager->isActivation(1)) {
                    return $this->failed('管理员激活失败！');
                }
            } else {
                if (!$Manager->isActivation(0)) {
                    return $this->failed('管理员取消激活失败！');
                }
            }
    ...
    public function qq()
    {
        if (Request::get('id')) {
            ...
            if (!$Manager->qq('')) {
                return $this->failed('管理员QQ解除绑定失败！');
            }
            return $this->success(Config::get('app.prev_url'));
        } else {
            return $this->failed('非法操作！');
        ...
        if (Request::get('id')) {
            ...
            if (Request::isPost()) {
                return $Manager->remove() ?
                    $this->success(Request::post('prev'), '管理员删除成功！') :
                    $this->failed('管理员删除失败！');
            }
    ...
    private function level1($id = 0)
    {
        ...
        foreach ($this->level as $key => $value) {
            $html .= '<option value="' . $key . '" ' . ($key == $id ? 'selected' : '') . ' class="' . $value[0] . '">' .
                $value[1] . '</option>';
        }
    ...
    private function level2($id = 0)
    {
        ...
        foreach ($this->level as $key => $value) {
            $html .= '<div class="radio-box"><label class="' . $value[0] .
                '"><input type="radio" name="level" value="' . $key . '" ' . ($key == $id ? 'checked' : '') . '>' .
                $value[1] . '</label></div>';
        }
        View::assign(['Level' => $html]);
    }
    ...
    {
        $html = '';
        foreach ($this->isActivation as $key => $value) {
            $html .= '<option value="' . $key . '" ' . ($key == $id ? 'selected' : '') . ' class="' . $value[0] . '">' .
                $value[1] . '</option>';
        }
    ...
    {
        $html = '<div class="radio-box"><label class="red"><input type="radio" name="is_activation" value="1" ' .
            ($id == 1 ? 'checked' : '') . '>是</label></div><div class="radio-box"><label class="green">' .
            '<input type="radio" name="is_activation" value="0" ' . ($id == 0 ? 'checked' : '') . '>否</label></div>';
        View::assign(['IsActivation' => $html]);
    }
    ...
    {
        $html = '';
        foreach ($this->orderPermit as $key => $value) {
            if ($key) {
                $html .= '<option value="' . $key . '" ' . ($key == $id ? 'selected' : '') . ' class="' . $value[0] .
                    '">' . $value[1] . '</option>';
            }
