        ...
        if (Request::isPost()) {
            ...
            if ($object) {
                ...
                foreach ($object as $value) {
                    $object2 = $Manager->one($value['manager_id']);
                    $output .= "\r\n" . '"' . ($object2 ? $object2['name'] : '此管理员已被删除') . '","' . $value['ip'] .
                        ' -- ' . $QQWry->getAddr($value['ip']) . '","' . dateFormat($value['date']) . '",';
                }
            }
            ...
            if (file_put_contents(ROOT_PATH . '/' . $file, $output)) {
                ...
                return $this->success(
                    '',
                    '文件生成成功！<a href="' . Config::get('app.web_url') . $file .
                    '">下载</a> <a href="javascript:" onclick="window.parent.addTab(\'' .
                    Route::buildUrl('/output/index') . '\',\'导出的数据\')">去管理文件</a> <a href="' .
                    Route::buildUrl('/' . parse_name(Request::controller()) . '/index') . '">返回</a>',
                    0,
                    2
                );
            } else {
