...
class TemplateStyle extends Base{
	...
	public function delete(){
		if (Request::get('id')){
			if (Config::get('app.demo') && Request::get('id')<=12) return $this->failed('演示站，id<=12的模板样式无法删除！');
			$TemplateStyle = new model\TemplateStyle();
			if (!$TemplateStyle->one()) return $this->failed('不存在此模板样式！');
			if (Request::isPost()) return $TemplateStyle->remove() ? $this->success(Request::post('prev'),'模板样式删除成功！') : $this->failed('模板样式删除失败！');
