...
class Permit extends Base
{
    ...
    public function isDefault()
    {
        if (Request::get('id')) {
            ...
            $object = $Permit->one();
            if (!$object) {
                return $this->failed('不存在此权限！');
            }
            if ($object['is_default'] == 0) {
                if (!$Permit->isDefault(1)) {
                    return $this->failed('设置默认权限失败！');
                }
            } else {
                if (!$Permit->isDefault(0)) {
                    return $this->failed('取消默认权限失败！');
                }
            }
...
return [';
        if ($object) {
            foreach ($object as $value) {
                $output .= "
    '" . $value['controller'] . "' => [
        '" . strtolower($value['action']) . "' => " . $value['id'];
                $object2 = $Permit->all3($value['id']);
                if ($object2) {
                    foreach ($object2 as $v) {
                        $output .= ",
        '" . strtolower($v['action']) . "' => " . $v['id'];
                    }
                }
                $output .= '
    ],';
...
';
        return file_put_contents(
            ROOT_PATH . '/' . Config::get('app.config_path_admin') . '/permit_manage.php',
            $output
        ) ?
            $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '配置文件更新成功！') :
            $this->failed('配置文件更新失败！');
    }
}
