...
return [
	...
	'alipay_merchant_private_key'=>'".str_replace("'","\'",Request::post('alipay_merchant_private_key'))."',  //支付宝应用私钥
	...
	'mail_order_content'=>'".str_replace("'","\'",Request::post('mail_order_content',NULL,'stripslashes'))."',  //订单提醒邮件内容
	'mail_pay_subject'=>'".str_replace("'","\'",Request::post('mail_pay_subject'))."',  //支付提醒邮件标题
	'mail_pay_content'=>'".str_replace("'","\'",Request::post('mail_pay_content',NULL,'stripslashes'))."',  //支付提醒邮件内容
	'mail_send_subject'=>'".str_replace("'","\'",Request::post('mail_send_subject'))."',  //发货提醒邮件标题
	'mail_send_content'=>'".str_replace("'","\'",Request::post('mail_send_content',NULL,'stripslashes'))."',  //发货提醒邮件内容
	'install_time'=>'".Config::get('system.install_time')."',  //安装时间
				...
				if (file_put_contents(ROOT_PATH.'/'.Config::get('app.config_path').'/system.php',$output)){
					if (Request::post('www',0)!=Config::get('system.www') || Request::post('https',0)!=Config::get('system.https')){
  ...
  RewriteRule ^(.*)$ index.php'.(strstr(php_uname('s'),'Windows') ? ' [L,E=PATH_INFO:$1]' : '/$1 [QSA,PT,L]').'
					...
					if (Request::post('manage_enter') != Config::get('system.manage_enter')){
						return rename(ROOT_PATH.'/'.Config::get('system.manage_enter'),ROOT_PATH.'/'.Request::post('manage_enter')) ? $this->success(Config::get('app.web_url').Request::post('manage_enter'),'后台入口修改成功，即将跳转到新入口！',0,1) : $this->failed('后台入口重命名失败，请检查系统根目录权限！');
					}else{
	...
	public function wechatIp(){
		include ROOT_PATH.'/extend/wechat/WechatExtra.class.php';
		$Wechat = new WechatExtra(['appid'=>Config::get('system.wechat_appid'),'appsecret'=>Config::get('system.wechat_appsecret')]);
		$Wechat->checkAuth();
	...
	private function skin($id=0){
		$html = '';
		foreach (['default'=>'黑色','blue'=>'蓝色','green'=>'绿色','red'=>'红色','yellow'=>'黄色','orange'=>'橙色'] as $key=>$value){
