...
class Template extends Base{
	...
	public function update(){
		if (Request::get('id')){
			...
			if (Request::isPost()){
				if (Config::get('app.demo')) if (in_array(Request::get('id'),[1,2,3])) return $this->failed('演示站，id为1、2、3的模板无法修改！');
	...
	public function delete(){
		if (Request::get('id')){
			if (Config::get('app.demo')) if (in_array(Request::get('id'),[1,2,3])) return $this->failed('演示站，id为1、2、3的模板无法删除！');
			$Template = new model\Template();
