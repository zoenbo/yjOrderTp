...
class ProductSort extends Base
{
    ...
    public function add()
    {
        if (Request::isPost()) {
            ...
            if (is_numeric($object)) {
                return $object > 0 ?
                    $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '产品分类添加成功！') :
                    $this->failed('产品分类添加失败！');
            } else {
    ...
    public function update()
    {
        if (Request::get('id')) {
            ...
            if (!$object) {
                return $this->failed('不存在此产品分类！');
            }
            if (Request::isPost()) {
                $object = $ProductSort->modify();
                return is_numeric($object) ?
                    $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '产品分类修改成功！') :
                    $this->failed($object);
            }
    ...
    public function delete()
    {
        if (Request::get('id')) {
            ...
            if (Request::isPost()) {
                return $ProductSort->remove() ?
                    $this->success(Request::post('prev'), '产品分类删除成功！') :
                    $this->failed('产品分类删除失败！');
            }
