...
class Logistics extends Base
{
    ...
    public function add()
    {
        if (Request::isPost()) {
            ...
            if (is_numeric($object)) {
                return $object > 0 ?
                    $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '物流添加成功！') :
                    $this->failed('物流添加失败！');
            } else {
    ...
    public function multi()
    {
        if (Request::isPost()) {
            ...
            return is_numeric($object) ?
                $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '物流批量添加成功！') :
                $this->failed($object);
        }
        return $this->view();
    }
    ...
    public function update()
    {
        if (Request::get('id')) {
            ...
            if (!$object) {
                return $this->failed('不存在此物流！');
            }
            if (Request::isPost()) {
                $object = $Logistics->modify();
                return is_numeric($object) ?
                    $this->success(Route::buildUrl('/' . parse_name(Request::controller()) . '/index'), '物流修改成功！') :
                    $this->failed($object);
            }
    ...
    public function delete()
    {
        if (Request::get('id')) {
            ...
            if (Request::isPost()) {
                return $Logistics->remove() ?
                    $this->success(Request::post('prev'), '物流删除成功！') : $this->failed('物流删除失败！');
            }
