...
class OrderState extends Base{
	...
	public function isDefault(){
		if (Request::get('id')){
			...
			$OrderState = new model\OrderState();
			if (!$OrderState->one()) return $this->failed('不存在此订单状态！');
			if (!$OrderState->isDefault()) return $this->failed('设置默认订单状态失败！');
	...
	public function delete(){
		if (Request::get('id')){
			...
			$OrderState = new model\OrderState();
			if (!$OrderState->one()) return $this->failed('不存在此订单状态！');
			if (Request::isPost()) return $OrderState->remove() ? $this->success(Request::post('prev'),'订单状态删除成功！') : $this->failed('订单状态删除失败！');
