...
class SubOrder extends Base
{
        ...
        if (Request::isPost()) {
            ...
            if (is_array($object)) {
                ...
                if (Request::post('pay') == 2) {
                    return $this->success(Config::get('app.web_url') . Config::get('system.index_php') .
                        'pay/alipay/oid/' . $object['order_id'] . '.html');
                } elseif (Request::post('pay') == 6) {
                    return $this->success(Config::get('app.web_url') . Config::get('system.index_php') .
                        'pay/wxpay/oid/' . $object['order_id'] . '.html');
                }
                return $this->success('', $object['success'], 0, 2);
            } else {
        ...
        if ($Smtp->total() > 0) {
            ...
            if ($object2) {
                try {
                    ...
                    foreach (
                        array_unique(
                            array_filter(explode(',', Config::get('system.admin_mail') . ',' . $object['admin_mail']))
                        ) as $value
                    ) {
    ...
    private function mail($content, $object)
    {
        return str_replace([
            ...
            isset($object['referrer']) ? '<a href="' . $object['referrer'] . '" target="_blank">' .
                $object['referrer'] . '</a>' : '直接进入',
