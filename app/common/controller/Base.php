...
class Base{
	...
	protected function error($tip='',$second=5,$type=0,$url=''){
		$a = $type ? '如果您的浏览器没有自动跳转，请点击这里' : '点击这里返回上一页';
		$html = '<!doctype html><html lang="zh-cn"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,minimum-scale=1.0,maximum-scale=1.0"><title>'.Config::get('system.web_name').'-提示</title><base href="'.Config::get('app.web_url').'"><script type="text/javascript" src="public/base/jquery.js?'.staticCache().'"></script><script type="text/javascript" src="public/base/H-ui.admin/h-ui/js/H-ui.min.js?'.staticCache().'"></script><script type="text/javascript" src="public/base/H-ui.admin/h-ui.admin/js/H-ui.admin.js?'.staticCache().'"></script><link rel="stylesheet" type="text/css" href="public/base/H-ui.admin/h-ui/css/H-ui.min.css?'.staticCache().'"><link rel="stylesheet" type="text/css" href="public/base/styles/Tip.css?'.staticCache().'"><script type="text/javascript">var ThinkPHP = {\'H-ui-skin\' : \''.Config::get('system.manage_skin').'\'};</script></head><body><div class="tip"><h3>提示</h3><div><h4 class="failed">'.$tip.'</h4>';
		if ($type == 0){
