$(function(){
	...
	function getList(){
		let str = '',
			$proSelectOption = $('.pro select option'),
			pro = $proSelectOption.length ? $proSelectOption : $('.pro label');
		for (let i=0;i<22;i++){
			let addressRand = Math.floor(Math.random()*22+1)-1,
				...
				productRand = Math.floor(Math.random()*pro.length+1)-1;
	...
	let $new = $('.new');
	if ($new.length){
		let marquee = ['张**（130****3260）在1','李**（136****7163）在3','赵**（139****1955）在5','刘**（180****6999）在2','张**（151****2588）在4','王**（133****4096）在6'],
			marquee1 = 0,
			marquee2 = 1,
			marquee3 = 2,
			$proSelectOption = $('.pro select option'),
