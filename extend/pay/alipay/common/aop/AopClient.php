...
class AopClient
{
    ...
    {
        $sHtml = '<form id="alipay_submit" name="alipay_submit" action="' . $this->gatewayUrl . '?charset=' .
            trim($this->postCharset) . '" method="POST">';
        foreach ($para_temp as $key => $val) {
            if (false === $this->checkEmpty($val)) {
                $sHtml .= '<input type="hidden" name="' . $key . '" value="' . str_replace('"', '&quot;', $val) . '">';
            }
        }
        return $sHtml . '<input type="submit" value="ok" style="display:none;"></form><script type="text/javascript">' .
